...
Define_Module(DotnetCoreApp);

simsignal_t DotnetCoreApp::rttSignal = registerSignal("rtt");
simsignal_t DotnetCoreApp::numLostSignal = registerSignal("numLost");
simsignal_t DotnetCoreApp::numOutOfOrderArrivalsSignal = registerSignal("numOutOfOrderArrivals");
simsignal_t DotnetCoreApp::pingTxSeqSignal = registerSignal("pingTxSeq");
simsignal_t DotnetCoreApp::pingRxSeqSignal = registerSignal("pingRxSeq");

#define TIMER_KIND 999


DotnetCoreApp::DotnetCoreApp()
{
    timer = nullptr;
}
...
void DotnetCoreApp::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        adapter = check_and_cast<DotnetApplicationAdapter*>(module);
...
void DotnetCoreApp::handleMessage(cMessage *msg)
{
    if (!isNodeUp()) {
...
void DotnetCoreApp::processMsg(ExternalAppPayload* msg)
{
...
bool DotnetCoreApp::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void DotnetCoreApp::startSendingPingRequests()
{
...
void DotnetCoreApp::stopSendingPingRequests()
{
    lastStart = -1;
    sendSeqNo = expectedReplySeqNo = 0;
...
bool DotnetCoreApp::isNodeUp()
{
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

bool DotnetCoreApp::isEnabled()
{
    //return par("destAddr").stringValue()[0] && (count == -1 || sentCount < count);
    return true;
}

void DotnetCoreApp::countPingResponse(int bytes, long seqNo, simtime_t rtt)
{
...
unsigned long DotnetCoreApp::getNodeId()
{
    return nodeId;
}


void DotnetCoreApp::sendMsg(unsigned long dest, int numBytes, int msgId)
{
    Enter_Method("send");
...
void DotnetCoreApp::wait(simtime_t duration)
{
    Enter_Method("wait");
